...
function processSum(nl, cb) {
  return cb(nl.reduce((accum, item) => {
    return accum + item;
  }, 0));
}
...
function getFullNames(runners) {
  const fullNames = [];
  runners.forEach((runner) => { // starts a for loop to run through runners storing runner
    fullNames.push(`${runner.last_name}, ${runner.first_name}`); // takes each runner in for loop and stores firstname last name
  });
  return fullNames; // returns full name in single line
}
